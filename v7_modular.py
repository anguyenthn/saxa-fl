...
###
""" ADD READ REQ FILE FOR PACKAGES ???????? what pip installs """
###

###
# ADD PATHS / CONFIGURATIONS

DATA_DIR = Path("/Users/an/Downloads/Projects/NFL/Data")                  # Put your path here 
INPUT_CSV = DATA_DIR / "df_merged.csv"                    # If user already has CSV from nflverse data then they can input it here
OUT_DIR = Path(".")
OUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_SEED = 20007
TEST_SIZE   = 0.25       # Explain why we used .25 rather than 70/30, or 80/20

###
# ESTABLISH MODEL TARGET & SCOPE

TARGET = None
RUN_ALL_TARGETS = True          # when True, loop over all numeric targets 
EXCLUDED_TARGETS = {"isaway", "intl", "is_thursday"}  # Do not use isaway, intl or is_thursday as targets
TRAIN_BY_POSITION = False
YEARS = range(2021, 2025)       # set year range. Upper end is non-inclusive

SEASON_AVG_KIND = "loo"          # Baseline strategy for per-player seasonal context: {"loo","prior","season"}
...
# Goal: Build a single per-player-per-game table df from API & official CSVs.
# DS intent: unify schemas, align team IDs, attach schedule/weather/travel context.
"""
...
""" DOUBLE CHECK NECESSITY OF THESE FUNCTIONS. IN PLACE FOR ERROR REDUCTION FOR FUTURE DATA UPLOAD"""
...
"""
# FEATURE ENGINEERING
# Goal: create usage/efficiency ratios + context encodings for RF models
# Domain-informed, leakage-safe predictors; fill missing data
...
def make_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # TRAVEL / REST TRAVEL (skips if missing)
    if KEEP_REST_TRAVEL:
    ...
    # Create efficiency/usage ratios based on time on field/usage in offense (must keep)
    ...
    # Reduces roof variable to open / closed (assumes anything not fully outdoors is closed)
    def roof_bin(v):
    ...
    # Alpha codes for stadium/surface
    ...
    if ADD_REC_TEAM_TGT_SHARE:
        try:
            ...
            d["rec_team_tgt_share"] = np.where(
                d["position_group"].isin(["WR","RB", "TE"]),
...
# CREATE SEASONAL AVERAGE SCHEMES
# What: Compute per-player-season baselines via Leave-One-Out / Prior Games / Entire Season mean
...
def season_avg(df: pd.DataFrame, kind: str = SEASON_AVG_KIND) -> pd.DataFrame:
    ...
    # LEAVES OUT THE CURRENT WEEK (ROW) WHEN CREATING THE SEASONAL AVERAGE. 
    if k == "loo":
        out = df.copy()
    ...
    # SEASON AVG OF GAMES UP TO, NOT INLCUDING CURRENT WEEK (ROW). WEEK 1 WILL NOT RETURN A VALUE. 
    if k == "prior":
    ...
    # COMPUTES THE ENTIRE SEASON AVERAGE
    if k == "season":
        out = df.copy()
...
def add_delta_binaries(df_now: pd.DataFrame, df_base: pd.DataFrame) -> pd.DataFrame:
    ...
    "surface_code", "stadium_id_num", "roof_closed", "rest_days", "travel_distance_home",
    "travel_distance_away", "lead_changes", "team_special_team_snaps", "special_team_snaps", "team_defensive_snaps",
    "team_offensive_snaps", "offensive snaps", "game_id", "is_thursday", "intl",
    "week_after_intl", "extended_away_games", "is_international", "isaway", "wind",
    "temp", "div_game", "home_rest", "away_rest", "gsis",
    "old_game_id", "overtime", "total", "result", "home_score",
    "away_score", "jersey_number", "defensive_snaps", "sack_fumbles_lost",
    "passing_first_downs", "passing_2pt_conversions", "rushing_first_downs", "rushing_fumbles_lost",
    "receiving_fumbles_lost", "receiving_first_downs", "receiving_2pt_conversions", "yards_after_catch", # duplicate YAC column
    "player_name_flat"} # These columns do not need to be compared to season averages, these are game-level not indidivually related stats. If individually related then it's heavily tied to game script
    ...
    pairs = [(c, f"{c}_season_avg") for c in num_now if c not in EXCLUDE_FROM_BINARIES and f"{c}_season_avg" in m.columns]
...
def train_and_explain(X: pd.DataFrame, y: pd.Series, target_name: str):
    ...
    # SHAP TO REDUCE FEATURES USED IN MODEL TO REDUCE NOISE
    shap_ok = False
    if HAS_SHAP:
        try:
...
if __name__ == "__main__":
    ...
    if RUN_ALL_TARGETS:
        # Derive candidate targets from numeric cols, drop excluded ones
        candidate_targets = [c for c in X_full.columns if c not in EXCLUDED_TARGETS]

        if TRAIN_BY_POSITION:
            segments = {
                "pass": dfB["position_group"] == "QB",
                "rush": dfB["position_group"] == "RB",
                "rec": dfB["position_group"].isin(["WR","RB", "TE"]),
            }
            for seg_name, mask in segments.items():
                sec(f"Train + explain for segment: {seg_name} — ALL TARGETS")
                rows = dfB.index[mask]
                for tgt in candidate_targets:
                    if tgt not in dfB.columns:
                        continue
                    y = dfB.loc[rows, tgt]
                    # require at least 2 unique values
                    if y.dropna().nunique() < 2:
                        print(f"Skipping {tgt} in {seg_name}: <2 unique values.")
                        continue
                    X = X_full.loc[rows].drop(columns=[tgt], errors="ignore")
                    _ = train_and_explain(X, y, f"{tgt}_{seg_name}")
        else:
            sec("Train + explain — ALL TARGETS (no position segments)")
            for tgt in candidate_targets:
                if tgt not in dfB.columns:
                    continue
                y = dfB[tgt]
                if y.dropna().nunique() < 2:
                    print(f"Skipping {tgt}: <2 unique values.")
                    continue
                X = X_full.drop(columns=[tgt], errors="ignore")
                _ = train_and_explain(X, y, tgt)
